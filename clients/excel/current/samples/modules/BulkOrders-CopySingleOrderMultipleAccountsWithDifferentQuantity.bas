...
Sub PlaceOrders()
    ...
    For OrderRow = 2 To 2
        ...
        For AccountRow = 2 To rAccounts.Rows.Count
            ...
            Quantity = rAccounts.Cells(RowIndex:=AccountRow, columnIndex:="B").Value
            ...
            Dim Price As Double: Price = wOrders.Cells(OrderRow, 7)
            Dim TriggerPrice As Double: TriggerPrice = wOrders.Cells(OrderRow, 8)
            Dim Target As Double: Target = wOrders.Cells(OrderRow, 9)
            Dim Stoploss As Double: Stoploss = wOrders.Cells(OrderRow, 10)
            Dim TrailingStoploss As Double: TrailingStoploss = wOrders.Cells(OrderRow, 11)
            Dim DisclosedQuantity As Long: DisclosedQuantity = 0
            Dim Validity As String: Validity = "DAY"
            Dim Amo As Boolean: Amo = wOrders.Cells(OrderRow, 12)
...
Sub PlaceOrdersManual()
    ...
    If MsgBox("Are you sure? Check the quantity against each account first.", vbYesNo) = vbNo Then
        Exit Sub
    End If
