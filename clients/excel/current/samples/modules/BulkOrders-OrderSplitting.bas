...
Sub PlaceOrders()
    ...
    ' Do validation (All orders must be validated upfront, otherwise we will end up placing only half of the orders)
    ' Validate accounts
    Temp = rAccounts.Cells(RowIndex:=2, columnIndex:="A").Value
    ...
    For OrderRow = 2 To rOrders.Rows.Count
        ...
        Dim SliceQty As Long: SliceQty = wOrders.Cells(OrderRow, 14)
        
        If (Quantity <= 0) Then
            MsgBox "Quantity for order #" & (OrderRow - 1) & " is invalid. No order was placed.", _
            vbCritical, "Invalid quantity"
            Exit Sub
        End If
        
        If (SliceQty <= 0) Then
            MsgBox "Slice quantity for order #" & (OrderRow - 1) & " is invalid. No order was placed.", _
            vbCritical, "Invalid slice qty"
            Exit Sub
        End If
        
        If (SliceQty > Quantity) Then
            MsgBox "Slice quantity [" & SliceQty & "] for order #" & (OrderRow - 1) _
            & " cannot be greater than quantity [" & Quantity & "]. No order was placed.", _
            vbCritical, "Invalid slice qty"
            Exit Sub
        End If
        
        Dim Slices As Integer: Slices = Quantity / SliceQty
        If ((Quantity Mod SliceQty) > 0) Then
            Slices = Slices + 1
        End If
        
        If (Slices > 10) Then
            MsgBox "You order #" & (OrderRow - 1) & " results in too many slices [" _
            & Slices & "]. It is too Risky, please contact support. No order was placed.", _
            vbCritical, "Too many slices"
            Exit Sub
        End If
    ...
    For OrderRow = 2 To rOrders.Rows.Count
        ...
        For AccountRow = 2 To rAccounts.Rows.Count
            ...
            Quantity = wOrders.Cells(OrderRow, 7)
            SliceQty = wOrders.Cells(OrderRow, 14)
            ...
            Dim RemainingQty As Long: RemainingQty = Quantity
            
            Do While RemainingQty > 0
            
                Dim OrderQty As Long: OrderQty = WorksheetFunction.Min(RemainingQty, SliceQty)
                
                orderId = PlaceOrderAdvanced(Variety, pseudoAccount, Exchange, _
                    Symbol, TradeType, OrderType, ProductType, OrderQty, Price, _
                ...
                RemainingQty = RemainingQty - OrderQty
            Loop
